          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
            ...
            <div className="relative w-full sm:w-auto sm:min-w-[300px]">
              <input
                ...
                placeholder="Поиск"
                className="w-full px-4 py-3 pl-12 bg-[#2A2529] border border-[#E62614] rounded-lg text-white placeholder-white/50 focus:outline-none focus:ring-2 focus:ring-[#E62614]"
              />
              <svg
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4 sm:gap-6 mb-8">
            {categories.map((category) => (
              <Link
                key={category.id}
                href={category.href}
                className="group relative bg-[#2A2529] border-2 border-[#E62614] rounded-xl p-4 sm:p-6 hover:border-[#E62614]/80 transition-all"
              >
                ...
                <h3 className="text-white text-sm font-medium text-center group-hover:text-[#E62614] transition-colors">
                  {category.name}
                </h3>
              </Link>
            ))}
